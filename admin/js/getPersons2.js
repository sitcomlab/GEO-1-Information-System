...
function sortRoom(obj) {
	for (var i = 0; i < obj.length; i++) {
		...
		if (obj[i].Name == "First aid room (Erste Hilfe)"
				...
				|| obj[i].Name == "Front-Office" || obj[i].Name == "ZDM (Zentrum Digitaler Medien)"
...
function createInstituteList(institute, logAction) {
	...
	var roomText = 'Room';
	var buttonText = 'Show Navigation';
	institute = sortByName(institute);
	...
	if (language === 'ger') {
		switch (oldAffiliation) {
			...
			case 'Dean':
				...
				break;
			case 'Library hall 0.10':
				newAffiliation = 'Bibliothek 0.10';
				break;
